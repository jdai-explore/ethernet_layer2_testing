...
def run(dut: str, tier: str, sections: str | None, output: str | None, db_url: str) -> None:
    ...
    # Build runner
    session_mgr, interface = create_test_components(config.dut_profile)
    validator = ResultValidator()
    runner = TestRunner(config, session_mgr, validator, interface=interface, progress_callback=progress)
